...
export default function OutlinePanel({ headings = [], onHeadingClick, activeHeadingId }) {
  if (headings.length === 0) {
    return (
      <aside className="h-full flex flex-col bg-white border-l border-gray-200 dark:bg-slate-800 dark:border-slate-700">
        <div className="px-4 py-3 border-b border-gray-100 dark:border-slate-700">
          <h3 className="flex items-center gap-2 text-xs font-semibold text-gray-400 uppercase tracking-wider dark:text-slate-500">
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center px-4">
            <FileText className="w-8 h-8 mx-auto mb-2 text-gray-200 dark:text-slate-600" />
            <p className="text-xs text-gray-300 dark:text-slate-600">使用标题格式后<br />自动生成大纲</p>
          </div>
        </div>
      </aside>
    );
    ...
    <aside className="h-full flex flex-col bg-white border-l border-gray-200 dark:bg-slate-800 dark:border-slate-700">
      <div className="px-4 py-3 border-b border-gray-100 dark:border-slate-700">
        <h3 className="flex items-center gap-2 text-xs font-semibold text-gray-400 uppercase tracking-wider dark:text-slate-500">
          <ListTree className="w-3.5 h-3.5" />
          大纲
          <span className="ml-auto font-normal text-gray-300 dark:text-slate-600">{headings.length}</span>
        </h3>
      </div>
